...
import type { ExtractedParamsWithConfidence } from './paramExtractor.service';
...
class ParamCacheService {
   ...
   * Returns: { params, confidences, lowConfidenceParams }
   */
  async getOrExtract(
    text: string,
    toolSlugs: string[],
    extractor: (text: string, params: ToolParam[]) => Promise<ExtractedParamsWithConfidence>
    ...
    const cachedResult = await globalCache.get<CachedExtractionResult>(extractionCacheKey);
    if (cachedResult) {
    ...
    for (const [key, value] of Object.entries(extracted.params)) {
      const confidence = extracted.confidences[key] || 0.5;
      ...
      if (value !== undefined && value !== null && value !== 'null') {
        ...
        if (!paramDef || PipelineValidator.validateParamValue(paramDef, value)) {
          ...
          if (confidence < 0.5) {
            lowConfidenceParams.push(key);
          }
        }
      }
    }

    const result: ExtractedParamsWithConfidence = {
      params: cleaned,
      confidences,
      lowConfidenceParams
    };
    ...
    // Cache the result
    const cacheableResult: CachedExtractionResult = {
      params: cleaned,
      confidences,
      lowConfidenceParams
    };
    
    await globalCache.set(extractionCacheKey, cacheableResult, {
